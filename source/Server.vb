...
Public Class Server
    ...
    Private ini As New IniReader(Replace(Mid(Environment.CommandLine, 1, InStrRev(Environment.CommandLine, "\") - 1) & "\simplehttpr.ini", Chr(34), ""))
    ...
    Public Sub HandleConnection()
        Try
            ...
            ElseIf InStr(sRequest, "default005.htm") <> 0 Then
                ...
                ' When these files are requested, a batch file with the same name is also executed on the server
            ElseIf InStr(sRequest, "run001.htm") <> 0 Then
                RunCommand(sLocalDir & "run001.bat")
                ssRequestedFile = "run001.htm"
            ElseIf InStr(sRequest, "run002.htm") <> 0 Then
                RunCommand(sLocalDir & "run002.bat")
                ssRequestedFile = "run002.htm"
            ElseIf InStr(sRequest, "run003.htm") <> 0 Then
                RunCommand(sLocalDir & "run003.bat")
                ssRequestedFile = "run003.htm"
            ElseIf InStr(sRequest, "run004.htm") <> 0 Then
                RunCommand(sLocalDir & "run004.bat")
                ssRequestedFile = "run004.htm"
            ElseIf InStr(sRequest, "run005.htm") <> 0 Then
                RunCommand(sLocalDir & "run005.bat")
                ssRequestedFile = "run005.htm"
